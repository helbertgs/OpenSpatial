...
extension Angle2D {
    ...
    /// Returns true if this angle is approximately equal to another within the given tolerance.
    ///
    /// - Parameters:
    ///   - other: The angle to compare against.
    ///   - tolerance: The maximum allowed difference in radians. Defaults to `sqrt(.ulpOfOne)`.
    /// - Returns: `true` if the absolute difference in radians is within the tolerance.
    /// - Complexity: O(1)
    public func isApproximatelyEqual(
        to other: Angle2D, tolerance: Double = Foundation.sqrt(.ulpOfOne)
    ) -> Bool {
        abs(radians - other.radians) <= tolerance
    }
}
...
extension Angle2D {
    ...
    private static func normalize(_ angle: Angle2D) -> Double {
    ...
    private static func sin(_ value: Double) -> Double {
    ...
    private static func cos(_ value: Double) -> Double {
        let x = normalize(value)
        let x2 = x * x
        let x4 = x2 * x2
        let x6 = x2 * x4
    ...
    private static func tan(_ value: Double) -> Double {
        sin(value) / cos(value)
    }
}
