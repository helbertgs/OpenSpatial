...
public struct Point3D: Copyable, Codable, Equatable, Hashable, Sendable {
    ...
    /// The point expressed as an array of x, y, and z values.
...
extension Point3D: ProjectiveTransformable3D {

    /// Returns a transformed copy of the point.
    ///
    /// Multiplies `[x, y, z, 1]` by the 4×4 projective matrix and divides
    /// by the resulting homogeneous `w` component.
    ///
    /// - Parameter transform: A projective transform to apply.
    /// - Returns: The transformed point.
    /// - Complexity: O(1)
    @inline(__always)
    public func applying(_ transform: ProjectiveTransform3D) -> Point3D {
        let m = transform.matrix
        let tx = m[0][0] * x + m[1][0] * y + m[2][0] * z + m[3][0]
        let ty = m[0][1] * x + m[1][1] * y + m[2][1] * z + m[3][1]
        let tz = m[0][2] * x + m[1][2] * y + m[2][2] * z + m[3][2]
        let tw = m[0][3] * x + m[1][3] * y + m[2][3] * z + m[3][3]
        guard tw != 0 else { return Point3D(x: tx, y: ty, z: tz) }
        return Point3D(x: tx / tw, y: ty / tw, z: tz / tw)
    }
}

extension Point3D: Clampable3D {

    /// Returns the point with coordinates clamped to the specified rectangle.
    ///
    /// - Parameter rect: The rectangle that defines the clamp volume.
    /// - Returns: A point that's clamped to the specified rectangle.
    /// - Complexity: O(1)
    @inline(__always)
    public func clamped(to rect: Rect3D) -> Point3D {
        Point3D(
            x: Swift.max(rect.min.x, Swift.min(rect.max.x, x)),
            y: Swift.max(rect.min.y, Swift.min(rect.max.y, y)),
            z: Swift.max(rect.min.z, Swift.min(rect.max.z, z))
        )
    }
}
