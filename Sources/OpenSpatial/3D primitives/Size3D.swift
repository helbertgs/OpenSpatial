...
@frozen public struct Size3D: Codable, Copyable, Equatable, Hashable, Sendable {
    ...
    /// The size expressed as an array of width, height, and depth values.
    public var vector: [Double] { [width, height, depth] }
...
extension Size3D: Primitive3D {
    ...
    public func applying(_ transform: AffineTransform3D) -> Size3D {
        Size3D(
            width: width * transform[0, 0],
            height: height * transform[1, 1],
            depth: depth * transform[2, 2]
        )
    }
}
...
extension Size3D: Volumetric {
    ...
    public func contains(_ other: Size3D) -> Bool {
        other.width <= width && other.height <= height && other.depth <= depth
    }
    ...
    public func contains(point: Point3D) -> Bool {
        point.x >= 0 && point.x <= width && point.y >= 0 && point.y <= height && point.z >= 0
            && point.z <= depth
    }
...
extension Size3D: Shearable3D {
    ...
    /// - Complexity: O(1)
    @inline(__always)
    public func sheared(_ shear: AxisWithFactors) -> Size3D {
        switch shear {
        case .xAxis(let ky, let kz):
            return Size3D(width: width + ky * height + kz * depth, height: height, depth: depth)
        case .yAxis(let kx, let kz):
            return Size3D(width: width, height: height + kx * width + kz * depth, depth: depth)
        case .zAxis(let kx, let ky):
            return Size3D(width: width, height: height, depth: depth + kx * width + ky * height)
        }
    }
}
