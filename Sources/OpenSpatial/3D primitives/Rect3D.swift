...
@frozen public struct Rect3D: Copyable, Codable, Equatable, Hashable, Sendable {
    ...
    public var integral: Rect3D {
        .init(
            ...
            size: Size3D(
                width: ceil(size.width), height: ceil(size.height), depth: ceil(size.depth)
            )
        )
    }
...
extension Rect3D: ProjectiveTransformable3D {

    /// Returns a transformed copy of the rectangle.
    ///
    /// Applies the transform to the origin and preserves the size.
    ///
    /// - Parameter transform: A projective transform to apply.
    /// - Returns: The transformed rectangle.
    /// - Complexity: O(1)
    @inline(__always)
    public func applying(_ transform: ProjectiveTransform3D) -> Rect3D {
        Rect3D(origin: origin.applying(transform), size: size)
    }
}
...
extension Rect3D: Shearable3D {
    ...
    ///
    /// Computes the axis-aligned bounding box of all 8 sheared corner points.
    ///
    /// - Parameter shear: The axis and shear factors.
    /// - Returns: The sheared rectangle.
    public func sheared(_ shear: AxisWithFactors) -> Rect3D {
        let corners = cornerPoints
        func shearPoint(_ p: Point3D) -> Point3D {
            switch shear {
            case .xAxis(let ky, let kz):
                return Point3D(x: p.x + ky * p.y + kz * p.z, y: p.y, z: p.z)
            case .yAxis(let kx, let kz):
                return Point3D(x: p.x, y: p.y + kx * p.x + kz * p.z, z: p.z)
            case .zAxis(let kx, let ky):
                return Point3D(x: p.x, y: p.y, z: p.z + kx * p.x + ky * p.y)
            }
        }
        let sheared = corners.map(shearPoint)
        let minX = sheared.min(by: { $0.x < $1.x })!.x
        let minY = sheared.min(by: { $0.y < $1.y })!.y
        let minZ = sheared.min(by: { $0.z < $1.z })!.z
        let maxX = sheared.max(by: { $0.x < $1.x })!.x
        let maxY = sheared.max(by: { $0.y < $1.y })!.y
        let maxZ = sheared.max(by: { $0.z < $1.z })!.z
        return Rect3D(
            origin: Point3D(x: minX, y: minY, z: minZ),
            size: Size3D(width: maxX - minX, height: maxY - minY, depth: maxZ - minZ)
        )
    }
}
...
extension Rect3D: Volumetric {
    ...
    public func intersection(_ other: Rect3D) -> Rect3D? {
        let newMin = Point3D(
            x: Swift.max(min.x, other.min.x), y: Swift.max(min.y, other.min.y),
            z: Swift.max(min.z, other.min.z))
        let newMax = Point3D(
            x: Swift.min(max.x, other.max.x), y: Swift.min(max.y, other.max.y),
        ...
        if newMin.x <= newMax.x && newMin.y <= newMax.y && newMin.z <= newMax.z {
            return Rect3D(
                origin: newMin,
                size: Size3D(
                    width: newMax.x - newMin.x, height: newMax.y - newMin.y,
                    depth: newMax.z - newMin.z))
        } else {
            return nil
        }
    }
    ...
    public func union(_ other: Rect3D) -> Rect3D {
        let newOrigin = Point3D(
            x: Swift.min(min.x, other.min.x), y: Swift.min(min.y, other.min.y),
            z: Swift.min(min.z, other.min.z))
        let newMax = Point3D(
            x: Swift.max(max.x, other.max.x), y: Swift.max(max.y, other.max.y),
            z: Swift.max(max.z, other.max.z))
        let newSize = Size3D(
            width: newMax.x - newOrigin.x, height: newMax.y - newOrigin.y,
            depth: newMax.z - newOrigin.z)
        return Rect3D(origin: newOrigin, size: newSize)
    }
}
