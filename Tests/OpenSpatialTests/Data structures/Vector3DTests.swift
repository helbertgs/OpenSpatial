...
struct Vector3DTests {
    ...
    @Test func testInitializationWithPoint() {
        let point = Point3D(x: 3.0, y: 5.0, z: 7.0)
        let vector = Vector3D(point)
        #expect(vector.x == 3.0)
        #expect(vector.y == 5.0)
        #expect(vector.z == 7.0)
    }
    ...
    // MARK: - Shearable3D tests (non-trivial values)

    @Test func testShearedXAxisNonTrivial() {
        // shear x by ky*y + kz*z: x' = x + ky*y + kz*z, y' = y, z' = z
        let v = Vector3D(x: 2, y: 3, z: 4)
        let result = v.sheared(.xAxis(yShearFactor: 0.5, zShearFactor: -1.0))
        #expect(result.x == 2 + 0.5 * 3 + (-1.0) * 4)  // 2 + 1.5 - 4 = -0.5
        #expect(result.y == 3)
        #expect(result.z == 4)
    }

    @Test func testShearedYAxisNonTrivial() {
        // shear y by kx*x + kz*z: y' = y + kx*x + kz*z
        let v = Vector3D(x: 5, y: 1, z: 2)
        let result = v.sheared(.yAxis(xShearFactor: 2.0, zShearFactor: -0.5))
        #expect(result.x == 5)
        #expect(result.y == 1 + 2.0 * 5 + (-0.5) * 2)  // 1 + 10 - 1 = 10
        #expect(result.z == 2)
    }

    @Test func testShearedZAxisNonTrivial() {
        // shear z by kx*x + ky*y: z' = z + kx*x + ky*y
        let v = Vector3D(x: 3, y: 4, z: 1)
        let result = v.sheared(.zAxis(xShearFactor: 1.5, yShearFactor: 0.25))
        #expect(result.x == 3)
        #expect(result.y == 4)
        #expect(result.z == 1 + 1.5 * 3 + 0.25 * 4)  // 1 + 4.5 + 1 = 6.5
    }

    @Test func testShearedZeroFactorsIsIdentity() {
        let v = Vector3D(x: 7, y: -3, z: 5)
        #expect(v.sheared(.xAxis(yShearFactor: 0, zShearFactor: 0)) == v)
        #expect(v.sheared(.yAxis(xShearFactor: 0, zShearFactor: 0)) == v)
        #expect(v.sheared(.zAxis(xShearFactor: 0, yShearFactor: 0)) == v)
    }

    @Test func testShearedNegativeFactors() {
        let v = Vector3D(x: 1, y: 1, z: 1)
        let result = v.sheared(.xAxis(yShearFactor: -2, zShearFactor: -3))
        #expect(result.x == 1 + (-2) * 1 + (-3) * 1)  // 1 - 2 - 3 = -4
        #expect(result.y == 1)
        #expect(result.z == 1)
    }
}