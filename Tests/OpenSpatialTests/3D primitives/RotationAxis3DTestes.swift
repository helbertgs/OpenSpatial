...
struct RotationAxis3DTests {
    ...
    @Test func testIsApproximatelyEqual() {
        let a = RotationAxis3D(x: 0.0, y: 1.0, z: 0.0)
        let b = RotationAxis3D(x: 0.0, y: 1.0, z: 0.0)
        #expect(a.isApproximatelyEqual(to: b))
    }

    @Test func testIsApproximatelyEqualWithTolerance() {
        let a = RotationAxis3D(x: 1.0, y: 0.0, z: 0.0)
        let b = RotationAxis3D(x: 1.0 + 1e-10, y: 0.0, z: 0.0)
